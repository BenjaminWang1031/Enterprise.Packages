...
ko.dependentObservable = function (evaluatorFunctionOrOptions, evaluatorFunctionTarget, options) {
    ...
    function evaluateImmediate() {
        if (_isBeingEvaluated) {
            ...
            // This is not desirable (it's hard for a Enterpriseer to realise a chain of dependencies might cause this, and they almost
                ...
                if (typeof eventName == "string") {
                    ko.utils.registerEventHandler(element, eventName, function (event) {
                        ...
                        try {
                            ...
                            if (handlerReturnValue !== true) { // Normally we want to prevent default action. Enterpriseer can override this be explicitly returning true.
...
ko.bindingHandlers['value'] = {
    ...
    'update': function (element, valueAccessor) {
        ...
        // We don't want to do a strict equality comparison as that is more confusing for Enterpriseers in certain cases, so we specifically special case 0 != "" here.
...
ko.bindingHandlers['html'] = {
    'init': function() {
        // Prevent binding on the dynamically-injected HTML (as Enterpriseers are unlikely to expect that, and it has security implications)
    ...
    function fixUpVirtualElements(contiguousNodeArray) {
        ...
        if (contiguousNodeArray.length > 2) {
            ...
            while (current !== last) {
                ...
                if (!current) // Won't happen, except if the Enterpriseer has manually removed some DOM elements (then we're in an undefined scenario)
                    return;
